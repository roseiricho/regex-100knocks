...
const RegexMatcher = ({id, inputText, type, answer}) => {
  ...
  const handleRegexChange = (e) => {
    const regexText = document.getElementById('regex-match-'+id)?.value || "";
    const regexOption = document.getElementById('regex-match-option-'+id)?.value || "";
    const replaceText = document.getElementById('regex-replace-'+id)?.value || "";
  ...
  return (
    <div>
      <div class="">
        ...
        <textarea type="text" id={'input-'+id} class="mb-5 py-3 px-4 w-full h-40 block border-textarea shadow-xs rounded-md text-sm focus:border-blue-500 focus:ring-blue-500 dark:bg-slate-900 dark:border-gray-700 dark:text-gray-400" readOnly value={inputText}></textarea>
      </div>
      <div class="my-5">
        ...
        <div class="flex rounded-md shadow-sm">
          ...
          <input type="text" id={'regex-match-option-'+id} onChange={handleRegexChange} autocomplete="off" name="hs-trailing-button-add-on-with-leading-and-trailing" class="py-3 px-4 block w-1/12 rounded-r-md border-input border-gray-200 shadow-sm rounded-0 text-sm focus:z-10 focus:border-blue-500 focus:ring-blue-500 dark:bg-slate-900 dark:border-gray-700 dark:text-gray-400" />
        </div>

        {
          type === 'replace' &&
            <div class="flex rounded-md shadow-sm mt-2">
              ...
              <input type="text" id={'regex-replace-'+id} onChange={handleRegexChange} placeholder='置換する文字列を入力してください．' autocomplete="off" name="hs-trailing-button-add-on-with-leading-and-trailing" class="py-3 px-4 block w-full border-input border-gray-200 shadow-sm rounded-0 text-sm focus:z-10 focus:border-blue-500 focus:ring-blue-500 dark:bg-slate-900 dark:border-gray-700 dark:text-gray-400" />
      ...
      <div class="flex justify-center w-full">
        <div class="w-full">
          ...
          <textarea type="text" id={'output-expected'+id} value={outputExpected} class="opacity-70 w-full h-40 block border-textarea pointer-events-none py-3 px-4 block bg-gray-50 border-gray-200 rounded-md text-sm focus:border-blue-500 focus:ring-blue-500 dark:bg-slate-900 dark:border-gray-700 dark:text-gray-400" disabled readOnly></textarea>
        </div>

        <div class="ml-5 w-full">
          <p class='mb-2 font-semibold text-left text-gray-800 transition dark:text-gray-200'>出力</p>
          <div class='relative'>
            <textarea type="text" id={'output-'+id} value={output} class={output == outputExpected ? "opacity-70 w-full h-40 block border-textarea-green pointer-events-none py-3 px-4 block rounded-md text-sm border-green-500 ring-blue-500 dark:bg-slate-900 dark:text-gray-400" : "opacity-70 w-full h-40 block border-textarea pointer-events-none py-3 px-4 block bg-gray-50 border-gray-200 rounded-md text-sm focus:border-blue-500 focus:ring-blue-500 dark:bg-slate-900 dark:border-gray-700 dark:text-gray-400"} disabled readOnly></textarea>
            {
          ...
          <div class='text-right'>
            <strong class='text-sm text-green-600 mt-2'>{output == outputExpected && 'LGTM!!'}</strong>
          </div>
        </div>
      </div>
    </div>
